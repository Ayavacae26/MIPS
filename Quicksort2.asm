...
quicksort:
	...
	addi $s0, $a0, 0		#load in array address
	addi $s1, $a1, 0		#load in p 
	addi $s2, $a2, 0		#load in r 
	...
	addi $s3, $v0, 0		#s3 = return value of Partition method
	addi $a0, $s0, 0		#a0 = array address
	addi $a1, $s1, 0		#a1 = p 
	addi $a2, $s3, -1		#a2 = q - 1 (r = q - 1)
	
	jal quicksort			#first quicksort recursion
	
	addi $a0, $s0, 0		#a0 = array address
	addi $a1, $s3, 1		#a1 = q + 1 (p = q + 1)
	addi $a2, $s2, 0		#a2 = r 
	
	jal quicksort			#second quicksort recursion 
...
loop:
beq	$s3, $s4, end2	# if j == r, end2 so what is life?
